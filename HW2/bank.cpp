...
int main(int argc, char *argv[]){
	...
	for(int i = 0; i < argc-1; i++){
		if(!(pthread_create(&(atm_threads[i]), NULL,
				(void*)(&operations), (void*)(&args[i]))){
			perror("Bank error: pthread_create failed");
			exit(1);
		}
	}
	
	if(!(pthread_create(&screen, NULL, (void*)(&print_screen), NULL))){
		perror("Bank error: pthread_create failed");
		exit(1);
	}
	
	if(!(pthread_create(&fees, NULL, (void*)(&bank_fees), NULL))){
		perror("Bank error: pthread_create failed");
		exit(1);
	}
	
	for(int i = 0; i < argc-1; i++){
		if(!(pthread_join(atm_treads[i], NULL))){
